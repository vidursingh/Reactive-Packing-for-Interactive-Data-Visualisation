  ...
  Engine: {
    ...
    update: function update(engine) {
      ...
      for (var i = 0; i < bodies.length; i += 1) {
        ...
        let bordering = bodyA.plugin.bordering;
        ...
        let details = bodyA.plugin.details;
        if(details != undefined)
          details(bodyA)


        let anchoring = bodyA.plugin.anchoring;
        if(anchoring != undefined){
          anchoring(bodyA)
        }

        let threshold = bodyA.plugin.threshold;
        if(threshold != undefined){
          threshold(bodyA)
        }

        let contain = bodyA.plugin.contain;
